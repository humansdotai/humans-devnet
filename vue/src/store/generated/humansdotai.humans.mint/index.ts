...
import { Minter } from "humansdotai-humans-client-ts/humansdotai.humans.mint/types"
import { Params } from "humansdotai-humans-client-ts/humansdotai.humans.mint/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: humansdotai.humans.mint initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.HumansdotaiHumansMint.query.queryParams()).data
		...
		async QueryInflation({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.HumansdotaiHumansMint.query.queryInflation()).data
		...
		async QueryAnnualProvisions({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.HumansdotaiHumansMint.query.queryAnnualProvisions()).data
